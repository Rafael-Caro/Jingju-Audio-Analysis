...
def loudnessHarmonics (fileName, dumpFile = False):
    ...
    harmLoudness = harmLoudness/np.max(harmLoudness)
...
def plotSyllablesLoudness(indexes, syllablesValues, loudnessTable):
    sections = []
    for i in range(len(indexes)):
        startTime = syllablesValues[indexes[i][1], 1]
        endTime = syllablesValues[indexes[i][1], 2]
        indStart = np.argmin(abs(loudnessTable[:,0]-startTime))
        indEnd = np.argmin(abs(loudnessTable[:,0]-endTime))
        section = loudnessTable[indStart:indEnd,:]
        sections.append(section)
    
    fig = plt.figure()
    for i in range(len(sections)):
        ax = fig.add_subplot(int(str(len(sections)) + '1' + str(i)))
        ax.plot(sections[i][:,0], sections[i][:,1])
